        ...
        ! M_d += torque
         ...
         DO i = 1, Src%Nnodes  ! the source nodes
            ...
            DO j = 1,2 ! number of nodes on dest 
               ...
               !write(*,*)
               !write(*,*) "Station = ",Src%Position(3,i)
               !write(*,*) "Src%Position =",Src%Position(:,i)- Dest%Position(:,jNode)
               !write(*,*) "Src%Disp =",SrcDisp%TranslationDisp(:,i)- DestDisp%TranslationDisp(:,jNode)
               !write(*,*) "Src%Position =",DisplacedPosition
               !write(*,*) "Source Forces = ", Src%Force(:,i)
               ...
               torque = CROSS_PRODUCT( DisplacedPosition, torque )               
               !write(*,*) "Torque = ", torque*MeshMap%MapLoads(i)%shape_fn(j)   
