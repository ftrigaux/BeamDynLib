...
Rhub       = 2.8
omega      = 7.5*9/(L+Rhub);
ncase      = [12,16,24,32,48,64] 
inputFile  = ["DTU10MW_BeamDyn_n%03d.dat"%(ix) for ix in ncase];
nx         = []
DynamicSolve=1
nt_loc     = 1
...
for ix in range(nnx):
    ...
    ax[0].plot(t,lvtip[ix],'-',label='nx = %d'%(nx[ix]));
...
for ix in range(nnx):
    #plt.plot(t,utip,'--',c='r',label="Spanwise");
        ax[1].plot(t,lwtip[ix],'-',label='nx = %d'%(nx[ix]));
...
for ix in range(nnx):
    ax[2].plot(t,lptip[ix]*np.rad2deg(1),'-',label='nx = %d'%(nx[ix]));
...
for ix in range(nnx):
    ...
    plt.semilogy(freq,np.abs(Px),label='nx = %d'%(nx[ix]))
    #plt.plot(freq[1:nt//2],np.abs(fft[1:nt//2]),label='nx = %d'%(nx[ix]));
